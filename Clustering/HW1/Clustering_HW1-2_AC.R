...
listings_subset <- listing_k2 %>% 
  ...
  mutate(host_response_rate=listings$host_response_rate2, host_listing_count=listings$host_listings_count, bathrooms=listings$bathrooms, bedrooms=listings$bedrooms, availability=listings$availability_365) %>%
...
pca_kmeans <- function(data){
  ...
  # Optimal Cluster Number
  plot(fviz_nbclust(pc$scores, kmeans, method = "wss",k.max=20))
...
pca_kmeans_cluster <- function(df,clusternumber){
  ...
  clus <- list()
  for( i in 1:clusternumber){
    clus[[i]] <-  combined_pca %>% filter(clus == i)
  }
  ...
  return(c(x<<-x, clu<<-clu, clus<<-clus, combined_pca <<- combined_pca))
}
...
plot_clusters <- function (clusternumber){
  ...
  plot(ggmap(map, extent="device") +
    ...
    print(paste('Avg Price:',mean(clusternumber$price)))
    ...
    print(paste('Avg Availability (days/year):',mean(clusternumber$availability)))
...
# Run PCA > HClust
pca_hclust(combined_pca[,-c(1,20:21,28:29)])
pca_hclust_cluster(combined_pca[,-c(1,20:21,28:29)],4)
x

####### OPTION 3: PCA > KMEANS (REPORTED) #######

# Run PCA > Kmeans
combined_pca <- combined
pca_kmeans(combined_pca[,-c(1,20:21,28:29)])
pca_kmeans_cluster(combined_pca[,-c(1,20:21,28:29)],5)

# View clusters
x

# Summary Stats/Plots
plot_clusters(clus[[5]])

# Export clusters to csv
fwrite(combined_pca, "Clustering/HW1/kmeansclusters.csv")
...
# Figure 3
ggmap(map, extent="device") +
  #geom_point(data=attractions, aes(x=lon, y=lat), color = 'yellow', size=6) +
  geom_point(data = clu[[1]], aes(x = lon, y = lat, colour = 'Cluster 1')) +
  geom_point(data = clu[[2]], aes(x = lon, y = lat, colour = 'Cluster 2')) +
  geom_point(data = clu[[3]], aes(x = lon, y = lat, colour = 'Cluster 3')) +
  geom_point(data = clu[[4]], aes(x = lon, y = lat, colour = 'Cluster 4')) +
  geom_point(data = clu[[5]], aes(x = lon, y = lat, colour = 'Cluster 5'))
...
ggmap(map2, extent="device") +
  geom_point(data=attractions, aes(x=lon, y=lat, colour = 'Top Attractions'), alpha=1, size=3) +
  geom_point(data = clu[[1]], aes(x = lon, y = lat, colour = 'Cluster 1'), alpha=0.1, size = clu[[1]]$price_bed*point) +
  geom_point(data = clu[[2]], aes(x = lon, y = lat, colour = 'Cluster 2'), alpha=0.1, size = clu[[2]]$price_bed*point) +
  # geom_point(data = clu[[3]], aes(x = lon, y = lat, colour = 'Cluster 3'), alpha=0.1, size = clu[[3]]$avgsscore*point) +
  # geom_point(data = clu[[4]], aes(x = lon, y = lat, colour = 'Cluster 4'), alpha=0.1, size = clu[[4]]$avgsscore*point) +
  # geom_point(data = clu[[5]], aes(x = lon, y = lat, colour = 'Cluster 5'), alpha=0.1, size = clu[[5]]$avgsscore*point) +
  # geom_point(data = clu[[6]], aes(x = lon, y = lat, colour = 'Cluster 6'), alpha=0.1, size = clu[[6]]$avgsscore*point) +
  # geom_point(data = clu[[7]], aes(x = lon, y = lat, colour = 'Cluster 7'), alpha=0.1, size = clu[[7]]$avgsscore*point) +
  theme(legend.position = "right") +
  scale_color_manual(name = element_blank(), values = colors)
...
ggmap(map2, extent="device") +
  geom_point(data=attractions, aes(x=lon, y=lat, colour = 'Top Attractions'), alpha=1, size=3) +
  geom_point(data = clu[[2]], aes(x = lon, y = lat, colour = 'Price per Bed'), alpha=0.2, size = clu[[2]]$price_bed*point) +
  theme(legend.position = "right") +
  scale_color_manual(name = element_blank(), values = colors)


#Figure 4
#filter clusters with below average rating
clu_filter<-as.data.frame(clu[[2]]) %>% filter(clu[[2]]$rating<mean(clu[[2]]$rating))
point <- 0.02
ggmap(map2, extent="device") +
  geom_point(data=attractions, aes(x=lon, y=lat, colour = 'Top Attractions'), alpha=1, size=2) +
  geom_point(data = clu[[2]], aes(x = lon, y = lat, colour = 'Availability'), alpha=0.2, size = clu[[2]]$availability*point) +
  geom_point(data = clu_filter, aes(x = lon, y = lat, colour = 'Below Average Ratings'), alpha=0.2, size = 4) +
  theme(legend.position = "right") +
  scale_color_manual(name = element_blank(), values = colors)
...
ggmap(map2, extent="device") +
  geom_point(data=attractions, aes(x=lon, y=lat, colour = 'Top Attractions'), alpha=1, size=3) +
  geom_point(data = clu[[1]], aes(x = lon, y = lat, colour = 'Price per Bed'), alpha=0.2, size = clu[[1]]$price_bed*point) +
  theme(legend.position = "right") +
  scale_color_manual(name = element_blank(), values = colors)

#Figure 6
clu_filter<-as.data.frame(clu[[1]]) %>% filter(clu[[1]]$rating<mean(clu[[1]]$rating))
point <- 0.015
ggmap(map2, extent="device") +
  geom_point(data=attractions, aes(x=lon, y=lat, colour = 'Top Attractions'), alpha=1, size=3) +
  geom_point(data = clu[[1]], aes(x = lon, y = lat, colour = 'Availability'), alpha=0.2, size = clu[[1]]$availability*point) +
  geom_point(data = clu_filter, aes(x = lon, y = lat, colour = 'Below Average Ratings'), alpha=0.2, size = 3) +
  theme(legend.position = "right") +
  scale_color_manual(name = element_blank(), values = colors)

#Bar charts
ggplot(clus[[2]], aes(property_type)) + 
  geom_bar(fill="steelblue") + xlab('Property Type') + ylab('Number of Listings') + 
  ggtitle('Cluster 2 Property Types') + 
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) + coord_flip()

ggplot(clus[[2]], aes(room_type)) + 
  geom_bar(fill="steelblue") + xlab('Room Type') + ylab('Number of Listings') + 
  ggtitle('Cluster 2 Room Types') + 
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) + coord_flip()

ggplot(clus[[1]], aes(property_type)) + 
  geom_bar(fill="steelblue") + xlab('Property Type') + ylab('Number of Listings') + 
  ggtitle('Cluster 1 Property Types') + 
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) + 
  coord_flip()

ggplot(clus[[1]], aes(room_type)) + 
  geom_bar(fill="steelblue") + xlab('Room Type') + ylab('Number of Listings') + 
  ggtitle('Cluster 1 Room Types') + 
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) +
  coord_flip()

#Summaries
mean(clu[[2]]$accommodates)
mean(clu[[2]]$beds)
mean(clu[[2]]$availability)